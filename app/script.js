...
window.addEventListener('click', (e) => {
    if(e.target.classList.contains('item-city')){
        const elementCity = htmlService.createEl('div', 'header__pop-city')
        elementCity.textContent = e.target.textContent
        headerPop.append(elementCity)
    }
})
...
function showLogIn(){
    document.querySelector('.profile-pop').classList.add('show')
}
function hideLogIn(){
    document.querySelector('.profile-pop').classList.remove('show')
}
...
function createLogin(login){
    const loginProfile = htmlService.createEl('div', 'header__pop-login')
